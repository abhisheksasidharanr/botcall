...
def generate_mock_data():
    ...
    # # Generate customers
    # for i in range(MOCK_DATA_SIZE["farmers"]):
    #     farmer = {
    #         "id": f"CUST{i:04d}",
    #         "name": f"Customer {i}",
    #         "phone": f"+1555{i:07d}",
...
def farmerexist(national_id=None):
    simulate_delay("database")   
    data = None
    with open('mock_data_outputs/data.json', 'r') as file:
            data = json.load(file)
    ...
    if national_id:
        farmer = next(
            (c for c in data["farmers"] if c["national_id"] == national_id), None
        )    
    else:
        return False

    return True if farmer else False


async def get_farmer(national_id=None):
    print(f'My national id is: {national_id}')
    """Look up a farmer by National ID."""
    ...
    return farmer if farmer else {"error": "Customer not found"}
...
async def register_farmer(national_id, name, state, crop, yield_qty, service):
    """Schedule a new appointment."""
    await simulate_delay("database")

    # Verify farmer exists
    farmer = farmerexist(national_id=national_id)
    if farmer:
        ...
        MOCK_DATA["farmers"].append(appointment)
        with open("mock_data_outputs/data.json", "w", encoding="utf-8") as file:
