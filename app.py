...
PROMPT_TEMPLATE = """You are Indian EUDR Registration System, a friendly and professional farmer service system. Your role is to assist farmer with farmer registration, farmer details, and general inquiries.
                            ...
                            try:
                                ...
                                if function_name in ["agent_filler", "end_call"]:
                                    ...
                                    if function_name=='create_farmer':
                                        sid = self.ws.id
                                        socketio.emit("register", json.dumps(result))
...
def farmers():
    data = None    
    with open('mock_data_outputs/data.json', 'r') as file:
            data = json.load(file)
    return (data)
...
def updateFarmer():
    ...
    national_id = farmer_data.get("national_id")
    eudr_status = farmer_data.get("eudr_status")
    for farmer in data["farmers"]:
        if farmer["national_id"] == national_id:
            farmer["eudr_status"] = eudr_status 
    with open('mock_data_outputs/data.json', "w") as file:
...
def run_async_voice_agent(agent, sid):
    try:
        ...
        try:
            ...
            logger.info(f"Voice agent task cancelled for SID {sid}")
        finally:
        ...
        logger.error(f"Error in voice agent thread for SID {sid}: {e}")
    finally:
        user_agents.pop(sid, None)
...
def handle_start_voice_agent(data=None):
    ...
    if sid not in user_agents:
        ...
        user_agents[sid] = voice_agent
        socketio.start_background_task(target=run_async_voice_agent, agent=voice_agent, sid=sid)

@socketio.on("stop_voice_agent")
def handle_stop_voice_agent(data=None):
    sid = data.get("sid")
    agent = user_agents.get(sid)
    if agent:
        agent.is_running = False
        if agent.loop and not agent.loop.is_closed():
            for task in asyncio.all_tasks(agent.loop):
                task.cancel()
        user_agents.pop(sid, None)
        
@socketio.on("disconnect")
def handle_disconnect():
    handle_stop_voice_agent()
